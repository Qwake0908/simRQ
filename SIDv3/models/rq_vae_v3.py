...
class RQVAEV3Model(LightningModule):
    """
    改进版 RQ-VAE 模型 (CosSimVQ)。
    
    结构：Encoder -> 余弦球面残差量化 (ResidualCosSimVQ) -> Decoder。
    更新方式：冻结随机码本，映射隐式码本并使用 Rotation Trick 传递梯度。
    度量：强制使用 L2 归一化和余弦相似度，避免大模型特征的模长干扰。
    """
